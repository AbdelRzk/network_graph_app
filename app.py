...
import numpy as np
import matplotlib.pyplot as plt  # Needed for the colormap
...
def index():
    if request.method == 'POST':
        try:
            ...
            colors = [(weight - min_weight) / (max_weight - min_weight) for weight in weights]  # Normalized weights

            fig = go.Figure()

            for edge, color in zip(G.edges(data=True), colors):
                x0, y0, z0 = pos_3d[edge[0]]
                x1, y1, z1 = pos_3d[edge[1]]
                
                fig.add_trace(go.Scatter3d(x=[x0, x1], y=[y0, y1], z=[z0, z1], mode='lines',
                                           line=dict(color=plt.cm.coolwarm(color), width=2)))
            ...
            fig.update_layout(title='3D Network Graph with PHQ-9 Score',
                              titlefont_size=16,
                              showlegend=False,
                              scene=dict(xaxis=dict(title='X'),
                                         yaxis=dict(title='Y'),
                                         zaxis=dict(title='PHQ-9 Score')))
